...
rmdir /mnt/btrfs-root 2>/dev/null

# 7. Initialize Snapper Config
echo "  Configuring Snapper for / ..."
snapper -c root create-config /

# 8. Correctly replace /.snapshots directory with @snapshots Btrfs Subvolume
echo " Mounting /@snapshots to /.snapshots..."
# Delete the folder generated by snapper create-config on root subvol
btrfs subvolume delete /.snapshots 2>/dev/null; or rm -rf /.snapshots
mkdir -p /.snapshots

# Update /etc/fstab if not present
...
if test -n "$CALLER_USER"
    ...
    chown -R $CALLER_USER:(id -g $CALLER_USER) $FISH_FUNC_DIR/arch.fish
end
